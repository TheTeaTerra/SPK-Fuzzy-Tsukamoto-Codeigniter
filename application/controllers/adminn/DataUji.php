...
class DataUji extends CI_Controller
{
    ...
    public function index()
    {
        ...
        if ($validation->run() == false) {
            ...
            'produksi_uji' => $post['produksi_uji'],
    ...
    public function nilaiKeanggotaan($id = null)
    {
        ...
        $keanggotaan = array("permintaan","produksi","penjualan");
    ...
    public function hitungFuzzy($id = null)
    {
        ...
        $keanggotaan = array('permintaan','produksi','penjualan','persediaan');
            ...
            foreach ($temp as $i1 => $value1) {
                ...
                foreach ($temp as $i2 => $value2) {
                    ...
                    foreach ($temp as $i3 => $value3) {
                        ...
                        $temp_produksi = $mnilai->getNilai($id, $keanggotaan[1].$value1);
                        $temp_penjualan = $mnilai->getNilai($id, $keanggotaan[2].$value2);

                        $permintaan = $temp_permintaan['nilai'];
                        $produksi = $temp_produksi['nilai'];
                        $penjualan = $temp_penjualan['nilai'];

                        $temp_min = array($permintaan,$produksi,$penjualan);

                        $nilaimin = min($temp_min);

                        $persediaan = 0;

                        $temp_persedianmin = $mset->getMin($keanggotaan[3]);
                        $persediaanMin = $temp_persedianmin[$keanggotaan[3].'min'];

                        $temp_persedianmax = $mset->getMax($keanggotaan[3]);
                        $persediaanMax = $temp_persedianmax[$keanggotaan[3].'max'];

                        if ($value3 == '_min') {
                            // rumus fuzzy saat persediaan dalam dataset min
                            $persediaan = $persediaanMax - (($persediaanMax - $persediaanMin) * $nilaimin);
                        } else {
                            // rumus fuzzy saat persediaan dalam dataset max
                            $persediaan = $persediaanMin + (($persediaanMax - $persediaanMin) * $nilaimin);
                        }
